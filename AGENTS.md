...
- `scripts/BoardView.gd`, `scripts/render/`, `scripts/input/`, `scripts/ui/`, and `scripts/ai/` handle presentation, input, interface flow, and computer opponents.
...
- Use PascalCase for scene/script filenames and `class_name`s (`GameState.gd`, `MatchRuntime.gd`); use `snake_case` for methods, variables, and signals (`record_pass`, `turn_completed`).
